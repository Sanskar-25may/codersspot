...
export default function LandingPage() {
  ...
  const marqReveal   = useScrollReveal({ variant: 'fade-down', threshold: 0.08 });
  const statsStagger = useStaggerReveal(4, { variant: 'fade-up', staggerMs: 90, threshold: 0.12 });
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ color: 'var(--text-primary)' }}>
      ...
      <section className="relative section-pad overflow-hidden" style={{ paddingTop: 'clamp(5rem, 12vw, 8rem)' }}>
        <div className="container-fluid flex flex-col items-center text-center">
          <div
            ...
            className={revealClass('zoom-in', heroReveal.isVisible, 'flex flex-col items-center gap-6 w-full max-w-4xl')}
          >
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              <a
                ...
                className="px-6 py-3 rounded-xl font-bold text-white flex items-center gap-2 hover:scale-[1.03] transition-all duration-200 shadow-lg"
                style={{ background: '#25D366', fontSize: 'clamp(0.8rem, 1.5vw, 0.95rem)' }}
              >
      ...
      <section className="border-y" style={{ background:'color-mix(in srgb, var(--bg-card) 85%, transparent)', borderColor:'var(--border-soft)' }}>
        <div ref={statsStagger.parentRef} className="container-fluid section-pad grid grid-cols-2 sm:grid-cols-4 gap-6">
          {landingData.stats.map((stat: any, idx: number) => (
            <div
              key={idx}
              className={revealClass('fade-up', statsStagger.items[idx]?.visible ?? false, 'text-center space-y-1')}
      ...
      {/* ─── MARQUEE ─── */}
      <section
        ref={marqReveal.ref as React.RefObject<HTMLElement>}
        className={revealClass('fade-down', marqReveal.isVisible, 'overflow-hidden section-pad')}
        ...
        <div className="container-fluid space-y-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {DEFAULT_FEATURES.map((feature, idx) => (
              <div key={idx}
                className={`p-6 rounded-2xl glass card-hover flex flex-col gap-3 ${feature.colSpan}`}
